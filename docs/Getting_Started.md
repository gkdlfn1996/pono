...
> 주의: su - postgres로 직접 들어가지 말고, 현재 셸에서 변수 확장 + su -c로 실행.
...
pg_hba.conf 파일을 수정하여 모든 인증 방식을 md5로 변경.(루트권한으로 실행해야함.)
