...
## **4.4 Publish용 노트 Subject & Header 설정하기**
노트를 게시할 때 사용될 **Subject**와 **Header Note**를 미리 설정하는 기능입니다.  
반복적인 입력을 자동화하여 작업 시간을 단축하고, 노트의 일관성을 유지할 수 있습니다.  
...
My Draft Note에 작성한 개별 임시 노트를 ShotGrid의 정식 노트로 발행하는 기능입니다.

![](./images/개별노트_퍼블리쉬.png)

My Draft Note에 작성한 임시 노트를 ShotGrid의 정식 노트로 발행하는 기능입니다. 이 기능의 핵심은, 게시 직전
별도의 팝업창에서 노트 내용과 첨부파일을 마지막으로 수정할 수 있다는 점입니다.

1. 게시 팝업창 열기   
게시할 노트가 있는 버전 카드에서 첨부파일 버튼 옆 `게시(↥)` 아이콘을 클릭합니다.

2. 최종 내용 확인 및 수정  
게시 팝업창은 ShotGrid에 전달될 최종 결과물을 미리 보여주는 공간입니다. 이곳에서 노트 내용(Content)을
마지막으로 수정하거나, 불필요한 첨부파일을 제거할 수 있습니다.
* To/CC: 버전의 담당 아티스트, 프로젝트의 슈퍼바이저 등이 자동으로 지정됩니다.
* Subject: 'Header & Subject Editor'에서 설정한 양식이 자동으로 적용됩니다.

3. ShotGrid에 게시하기  
하단의 `Publish to ShotGrid` 버튼을 누르면 게시가 시작됩니다. 게시가 진행되는 동안에는 로딩 아이콘이
표시되며, 완료될 때까지 다른 동작을 할 수 없습니다.

4. 게시 결과 확인  
* 성공 시: "Publish complete" 메시지가 나타난 후 팝업창이 자동으로 닫힙니다. My Draft Note에 있던 임시
    노트는 사라지고, 'Official ShotGrid Notes' 영역에서 해당 노트를 확인할 수 있습니다.
* 실패 시: 팝업창 내에 "Publish complete. ... Failed: 1" 과 같은 경고 메시지가 나타납니다. 이 경우, 팝업창을
    닫고 내용을 수정한 뒤 다시 시도해야 합니다.




1.  게시할 노트가 있는 버전 카드에서 첨부파일 버튼 옆 **`↥`** 버튼을 클릭합니다.
...
## **4.6 여러 노트 한번에 게시하기 (All Notes Publish)**
